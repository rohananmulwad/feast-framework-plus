...
const RestaurantMenu = () => {
  ...
  const fetchRestaurantData = async () => {
    try {
      ...
      } = await supabase.from("restaurants").select("*").eq("slug", slug).eq("is_active", true).single();
      ...
      } = await supabase.from("menu_categories").select("*").eq("restaurant_id", restaurantData.id).eq("is_active", true).order("display_order");
      ...
      const {
        data: itemsData,
        error: itemsError
      } = await supabase.from("menu_items").select("*").in("category_id", (categoriesData || []).map(c => c.id)).eq("is_available", true).order("display_order");
    ...
    backgroundColor: restaurant.background_color
      ...
      <div className="relative h-56 sm:h-64 md:h-80 lg:h-96 overflow-hidden">
        {restaurant.banner_image_url ? <>
            <img src={restaurant.banner_image_url} alt={restaurant.name} className="w-full h-full object-cover" />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 text-white animate-slide-up">
          <div className="container max-w-7xl mx-auto">
            <div className="flex flex-col sm:flex-row items-start sm:items-end gap-3 sm:gap-4">
              {restaurant.logo_url && <div className="relative">
                  <div className="absolute inset-0 bg-primary/30 blur-xl rounded-full" />
                  <img src={restaurant.logo_url} alt="" className="relative h-16 w-16 sm:h-20 sm:w-20 rounded-full border-4 border-white shadow-2xl object-cover transition-transform duration-300 hover:scale-110" />
                </div>}
              <div className="flex-1">
                <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold mb-2 drop-shadow-lg tracking-tight">
                  {restaurant.name}
                </h1>
                {restaurant.description && <p className="text-base sm:text-lg text-white/90 max-w-2xl leading-relaxed">
        ...
        <div className="container max-w-7xl mx-auto px-4 py-3 sm:py-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
            ...
            <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-4 text-sm">
              {restaurant.address && <div className="flex items-start sm:items-center gap-2 text-muted-foreground">
                  <MapPin className="h-4 w-4 mt-0.5 sm:mt-0 flex-shrink-0" />
                  <span className="line-clamp-1">{restaurant.address}</span>
                </div>}
              {restaurant.contact_phone && <div className="flex items-center gap-2 text-muted-foreground">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Button variant={dietFilter === "all" ? "default" : "outline"} size="sm" onClick={() => setDietFilter("all")} className="h-9 px-4 transition-all duration-300 hover:scale-105 font-medium">
                All
              </Button>
              <Button variant={dietFilter === "veg" ? "default" : "outline"} size="sm" onClick={() => setDietFilter("veg")} className="h-9 px-3 sm:px-4 gap-2 transition-all duration-300 hover:scale-105 font-medium" style={dietFilter === "veg" ? {
              backgroundColor: "#16a34a"
            } : {}}>
                <VegIcon />
                <span className="hidden sm:inline">Veg</span>
              </Button>
              <Button variant={dietFilter === "non-veg" ? "default" : "outline"} size="sm" onClick={() => setDietFilter("non-veg")} className="h-9 px-3 sm:px-4 gap-2 transition-all duration-300 hover:scale-105 font-medium" style={dietFilter === "non-veg" ? {
      ...
      <div className="container max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 md:py-12">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-3 space-y-8 sm:space-y-12">
            ...
            return <div key={category.id} id={`category-${category.id}`} className="scroll-mt-24">
                  <div className="mb-6">
                    ...
                    {category.description && <p className="text-muted-foreground leading-relaxed">{category.description}</p>}
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
                    {items.map((item, index) => <Card key={item.id} className="overflow-hidden border-border/50 hover:border-primary/50 hover:shadow-2xl hover:shadow-primary/5 transition-all duration-500 cursor-pointer group bg-gradient-to-br from-card to-card/80 animate-slide-up" style={{
                  ...
                  setQuantity(1);
                        ...
                        <CardContent className="p-0">
                          <div className="flex gap-3 sm:gap-4 p-3 sm:p-4">
                            {item.image_url && <div className="relative flex-shrink-0">
                                <img src={item.image_url} alt={item.name} className="w-20 h-20 sm:w-24 sm:h-24 rounded-lg object-cover transition-transform duration-500 group-hover:scale-110" />
                              ...
                              <div className="flex items-start gap-2 mb-2">
                                <div className="flex items-start gap-2 flex-1 min-w-0">
                                  <div className="flex-shrink-0 mt-0.5">
                                    {item.is_vegetarian || item.is_vegan ? <VegIcon /> : <NonVegIcon />}
                                  </div>
                                  <h3 className="font-semibold text-base sm:text-lg leading-tight group-hover:text-primary transition-colors duration-300">
                                    {item.name}
                                  </h3>
                                </div>
                                <span className="font-bold text-base sm:text-lg whitespace-nowrap flex-shrink-0" style={{
                            color: restaurant.theme_color
                          }}>
                                  ₹{item.price.toFixed(2)}
                                </span>
                              </div>
                              {item.description && <p className="text-xs sm:text-sm text-muted-foreground mb-2 sm:mb-3 line-clamp-2 leading-relaxed">
                                  {item.description}
                                </p>}
                              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                                {item.is_spicy && <Badge variant="destructive" className="text-xs px-2 py-0.5">
            ...
            {categories.length === 0 && <div className="text-center py-20 md:py-32">
      ...
      <Dialog open={!!selectedItem} onOpenChange={() => setSelectedItem(null)}>
        <DialogContent className="max-w-2xl p-0 gap-0 overflow-hidden animate-scale-in">
          {selectedItem && <div className="relative">
              {/* Close Button */}
              <button onClick={() => setSelectedItem(null)} className="absolute top-4 right-4 z-10 bg-black/50 hover:bg-black/70 text-white rounded-full p-2 transition-all duration-300 hover:scale-110 hover:rotate-90">
                <X className="h-5 w-5" />
              </button>

              {/* Image */}
              {selectedItem.image_url && <div className="relative h-64 md:h-80 overflow-hidden">
                  <img src={selectedItem.image_url} alt={selectedItem.name} className="w-full h-full object-cover animate-fade-in" />
                </div>}
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="flex items-start gap-3">
                  {selectedItem.is_vegetarian || selectedItem.is_vegan ? <VegIcon className="mt-1" /> : <NonVegIcon className="mt-1" />}
                  <h2 className="text-2xl md:text-3xl font-bold flex-1">{selectedItem.name}</h2>
                </div>

                {/* Dietary Badges */}
                {selectedItem.is_spicy && <Badge variant="destructive">
                    <Flame className="h-3 w-3 mr-1" />
                    Spicy
                  </Badge>}

                {/* Description */}
                {selectedItem.description && <p className="text-muted-foreground leading-relaxed">
